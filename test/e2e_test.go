...
func startCliCommand(t *testing.T, address string, clientName string, cliCommand string, cliArgs ...string) *startResult {
	...
	return &startResult{stdout: stdoutPipe, stderr: stderrPipe, cmd: cmd, done: done}
}
...
func executeCliCommand(t *testing.T, address string, clientName string, expectFailure bool, cliCommand string, cliArgs ...string) *executionResult {
	...
	// Read stdout and stderr concurrently to avoid deadlocks if buffers fill up.
	var readStdoutErr error
	var stdoutData string
	go func() {
		data, err := io.ReadAll(startRes.stdout)
		readStdoutErr = err
	...
	go func() {
		data, err := io.ReadAll(startRes.stderr)
		readStderrErr = err
	...
	case err := <-startRes.done:
		...
		if readStdoutErr != nil {
			t.Fatalf("failed to read CLI stdout: %v", readStdoutErr)
		}
		if readStderrErr != nil {
			t.Fatalf("failed to read CLI stderr: %v", readStderrErr)
		}
